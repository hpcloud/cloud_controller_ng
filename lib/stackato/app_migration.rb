      ...
      def validate_app_migration(app, domains, routes, service_instances, is_moving_org)
        ...
        if is_moving_org
          domains.each { |domain|
            ...
            if apps.size != 1 || !apps.include?(app)
              raise 'application domains are in use by more than one application'
            end
          }
        end
        ...
        routes.each { |route|
          if route.apps.size != 1 && !route.apps.include?(app)
            raise 'application routes are in use by more than one application'
          end
        }
        ...
        service_instances.each { |service_instance|
          ...
          if apps.size != 1 && !apps.include?(app)
            raise 'application service instances are in use by more than one application'
          end
        }
      end
    end
