...
if [ ! "$(ls -A $DATADIR)" ]; then
  ...
  su postgres sh -c "$BINDIR/postgres --single -D $DATADIR \
    -c config_file=$CONF" <<< "CREATE USER root WITH SUPERUSER PASSWORD 'postgres';"
fi

## cat $CONF

su postgres sh -c "$BINDIR/postgres -D $DATADIR \
  -c config_file=$CONF -c listen_addresses=*"
