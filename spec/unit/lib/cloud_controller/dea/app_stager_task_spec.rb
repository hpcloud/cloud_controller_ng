  ...
  describe Dea::AppStagerTask do
    ...
    describe "staging memory requirements" do
      context 'when the app memory requirement exceeds the staging memory requirement (1024)' do
        it 'should request a stager with the app memory requirement' do
          app.memory = 1025
          expect(stager_pool).to receive(:find_stager).with(app.stack.name, 1025, anything, 'default').and_return(stager_id)
          staging_task.stage
        end
      end
      ...
      context 'when the app memory requirement is less than the staging memory requirement' do
        it "requests the staging memory requirement" do
          ...
          expect(stager_pool).to receive(:find_stager).with(app.stack.name, 2048, anything, 'default').and_return(stager_id)
          staging_task.stage
        end
      end
    end
    ...
    describe "staging disk requirements" do
      ...
      context 'when the app disk requirement is less than the default (4096) staging disk requirement, and it wasnt overridden' do
        it "should request a stager with enough disk" do
          ...
          expect(stager_pool).to receive(:find_stager).with(app.stack.name, anything, 4096, 'default').and_return(stager_id)
          staging_task.stage
        end
      end
      ...
      context 'when the app disk requirement exceeds the staging disk requirement' do
        it "should request a stager with enough disk" do
          ...
          expect(stager_pool).to receive(:find_stager).with(app.stack.name, anything, 123, 'default').and_return(stager_id)
          staging_task.stage
        end
      end
    end
    ...
    describe "staging" do
      ...
      describe "reserve app memory" do
        before do
          allow(stager_pool).to receive(:find_stager).with(app.stack.name, 1025, 4096, 'default').and_return(stager_id)
        end
