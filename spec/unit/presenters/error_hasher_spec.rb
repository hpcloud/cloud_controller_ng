...
describe ErrorHasher do
  def expect_generic_message(sanitized_hash)
    ...
    expect(sanitized_hash['description']).to match(/\AAn unknown error occurred. Please contact your administrator, specifying error tracker ID \d+ along with this message.\Z/)
  end
  ...
  describe '#unsanitized_hash' do
    ...
    context 'with nil' do
      ...
      it 'returns a default hash' do
        ...
        expect(unsanitized_hash['description']).to match(/\ANo error. Please contact your administrator, specifying error tracker ID \d+ along with this message.\Z/)
      end
    end
