    ...
    describe "GET /v2/app_usage_events" do
      ...
      context "when filtering by after_guid" do
        ...
        it "maintains the after_guid in the next_url" do
          ...
          expect(last_response).to be_successful
          expect(decoded_response.fetch("next_url")).to eql("/v2/app_usage_events?after_guid=#{@event1.guid}&order-by=id&order-direction=asc&page=2&results-per-page=1")
        end
        ...
        it "maintains the after_guid in the prev_url" do
          ...
          expect(last_response).to be_successful
          expect(decoded_response.fetch("prev_url")).to eql("/v2/app_usage_events?after_guid=#{@event1.guid}&order-by=id&order-direction=asc&page=1&results-per-page=1")
        end
