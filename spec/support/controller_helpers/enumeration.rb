    ...
    describe "GET #{path}" do
      ...
      if prev_page
        it "should return a prev_url to page #{prev_page}" do
          expected_qp = "order-by=name&order-direction=asc&page=#{prev_page}&results-per-page=#{page_size}"
      ...
      if next_page
        it "should return next_url to page #{next_page}" do
          expected_qp = "order-by=name&order-direction=asc&page=#{next_page}&results-per-page=#{page_size}"
          expected_url = if base_path =~ /\?/
                           /#{base_path.sub("?", "\\?")}&#{expected_qp}/
                         else
                           /#{base_path}\?#{expected_qp}/
